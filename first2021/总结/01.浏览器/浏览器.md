...
2. get 请求在 url 中传递的参数是有长度限制的（**但是这个长度是由于浏览器限制的**），而 post 没有。
3. get 比 post 更不安全，因为参数直接暴露在 url 中，所以不能用来传递敏感信息。
4. get 请求只能进行 url 编码，而 post 支持多种编码方式。（**application/x-www-form-urlencoded、multipart/form-data、application/json、text/xml**）
...
> [参考二，详细](http://www.dailichun.com/2018/03/12/whenyouenteraurl.html)
...
1. CSRF攻击形式：
    ...
    ```特点：不需要访问a网站的前端，直接访问a网站的后端```

    ```
    例如：你登录a网站，并在本地存储cookie
    如果在没退出a网站的时候 不小心访问了恶意b网站，而且这个b网站需要你发一些请求等
    此时，你是携带cookie进行访问的，那么你的重在cookie里的信息就会被恶意网站捕捉到，那么你的信息就被盗用，导致一些不法分子做一些事情
    ```

2. 攻击防御：

 - 设置cooki时候+一个sameSite属性()
        ```
